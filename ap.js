...
async function send_telega2(event) {
  ...
  try {
   ...
   // console.error(error); //виведення помилки яку зафіксували
    massage_rezalt2.textContent = "произошла ошибка, подайте заказ ещё раз"; // виведення повідомлення коли запрос не успішний
      }
}
...
async function send_telega3(event) {
  ...
  try {
   ...
   // console.error(error); //виведення помилки яку зафіксували
    massage_rezalt3.textContent = "произошла ошибка, подайте заказ ещё раз"; // виведення повідомлення коли запрос не успішний
      }
}
